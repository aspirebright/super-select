...
export default class SuperSelect extends LightningElement {
    ...
    get showListbox() {
        return this.configTrack && this.configTrack.filteredSuggestedItems && this.configTrack.filteredSuggestedItems.length > 0 && (this.showList || this.configTrack.alwaysShowList);
    }
    ...
    triggerRequiredFieldValidation() {
        if (this.configTrack.required) {
            if (this.hadFocus && this.configTrack && this.configTrack.filteredSuggestedItems && this.configTrack.filteredSuggestedItems.length > 0 && !this.selectedItemsMap.size > 0) {
    ...
    @api
    resetSelection(itemsToRemove, errorMessage, messageType) {
        //itemsToRemove(jsonArray), errorMessage(String), messageType(String = toast/field-error)
        if (itemsToRemove) {
    ...
    @api
    resetRemoval(itemsToReSelect, errorMessage, messageType) {
        //itemsToReSelect(jsonArray), errorMessage(String), messageType(String = toast/field-error)
        if (itemsToReSelect) {
    ...
    processPicklist() {
        ...
        if (
            this.configTrack.sort !== undefined &&
            this.getStringDefault(this.configTrack.sort.order) !== "" &&
            this.getStringDefault(this.configTrack.sort.order) !== "custom" &&
            this.getStringDefault(this.configTrack.sort.field) !== ""
        ) {
    ...
    handleInputKeyUp(event) {
        ...
        if (this.getKeyCode(event) === 40 && this.configTrack.filteredSuggestedItems.length > 0 && listItems && listItems.length > 0) {
            listItems[0].focus();
        } else {
    ...
    handleSelection(event) {
        ...
        if (
            this.configTrack.onSelect &&
            this.configTrack.onSelect.notAllowedToBeSelected &&
            this.configTrack.onSelect.notAllowedToBeSelected.items &&
            this.configTrack.onSelect.notAllowedToBeSelected.items.length > 0
        ) {
    ...
    handleRemoveSelection(event) {
        ...
        if (
            this.configTrack.onRemove &&
            this.configTrack.onRemove.notAllowedToBeRemoved &&
            this.configTrack.onRemove.notAllowedToBeRemoved.items &&
            this.configTrack.onRemove.notAllowedToBeRemoved.items.length > 0
        ) {
    ...
    handleClearAllItems(event) {
        ...
        if (
            ...
            if (this.configTrack.onRemove.notAllowedToBeRemoved.items && this.configTrack.onRemove.notAllowedToBeRemoved.items.length > 0) {
                loop1: for (let i = 0; i < this.configTrack.onRemove.notAllowedToBeRemoved.items.length; i++) {
                    loop2: for (let j = 0; j < this.removedItems.length; j++) {
    ...
    handleListItemBlur(event) {
        if (this.showList === true) {
            ...
            if (relatedTarget !== undefined && relatedTarget !== null) {
                if (
                    (relatedTarget.tagName === "DIV" && relatedTarget.parentElement !== undefined && relatedTarget.parentElement.tagName === "LI") ||
                    (relatedTarget.tagName === "INPUT" && relatedTarget.dataset.id === "inputElement") ||
